...
export default function RegisterCandidateModal({ isOpen, onClose, adminToken }: RegisterCandidateModalProps) {
  ...
  const { mutate, isPending } = useMutation({
    ...
    onSuccess: () => {
      toast.success('Candidat inscrit avec succès !');
          ...
          >
            ...
            <div className="border-b border-amber-400/10 flex items-center justify-between sticky top-0 bg-[#0f0f1a]/95 backdrop-blur-md z-20" style={{ padding: s(5) }}>
              <div>
                ...
                <p className="text-xs text-white/40">Le candidat sera directement enregistré avec le statut approuvé.</p>
              </div>
              <button
                onClick={onClose}
            ...
            <div className="overflow-y-auto flex-1" style={{ padding: s(6) }}>
              <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: s(5) }}>
                ...
                <div className="flex flex-col items-center" style={{ gap: s(2), marginBottom: s(2) }}>
                  ...
                  <p className="text-[11px] text-white/40">Cliquez pour ajouter une photo</p>
                </div>
                ...
                <div>
                  ...
                  <textarea name="biography" required rows={3} className={inputCls} style={{ ...fieldStyle, resize: 'vertical' }}
